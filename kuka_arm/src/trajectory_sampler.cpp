    ...
    eef_group(GRIPPER_GROUP)
{
  ...
  while (ros::ok())
  {
    ...
    bool success = (move_group.plan(my_plan) == moveit_msgs::MoveItErrorCodes::SUCCESS);
    ...
    if (demo)
    {
      ...
      success = (move_group.execute(my_plan) == moveit_msgs::MoveItErrorCodes::SUCCESS);
    ...
    else
    {
      ...
      for (std::size_t i = 0; i < srv.response.points.size(); ++i)
      {
        ...
        bool worked = (move_group.move() == moveit_msgs::MoveItErrorCodes::SUCCESS);
    ...
    success = (move_group.move() == moveit_msgs::MoveItErrorCodes::SUCCESS);
    ...
    visual_tools.trigger();
    CloseGripper();
    ros::Duration(3.0).sleep();  // from Common Questions
    visual_tools.prompt("next step");
...
bool TrajectorySampler::OperateGripper(const bool &close_gripper)
{
  ...
  bool success = (eef_group.move() == moveit_msgs::MoveItErrorCodes::SUCCESS);
  return success;
}
